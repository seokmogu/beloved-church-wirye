...
- Development/staging must use separate Vercel, Neon, and Blob resources before destructive admin QA. Local dev must not connect to the production Neon branch or production Blob store unless the user explicitly approves a read-only diagnostic.
...
- Required production env names include `POSTGRES_URL`, `PAYLOAD_SECRET`, `NEXT_PUBLIC_SERVER_URL`, `PAYLOAD_SERVER_URL`, `PAYLOAD_PUBLIC_ORIGINS`, `CRON_SECRET`, `PREVIEW_SECRET`, `MANAGE_AUTH_SECRET`, and any enabled integration vars such as `R2_MEDIA_ACCESS_KEY_ID`, `R2_MEDIA_BUCKET`, `R2_MEDIA_ENDPOINT`, `R2_MEDIA_PUBLIC_URL`, `R2_MEDIA_SECRET_ACCESS_KEY`, `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID`, `OPENCLAW_API_URL`, and `OPENCLAW_GATEWAY_TOKEN`. Public `media` and private `gallery-media` must use separate R2 buckets and credentials. Do not print, commit, or copy secret values.
